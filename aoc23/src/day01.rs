...
/// Version 6.
///
/// Use `starts_with` checks instead, which turn out to be way faster - the assembly generated by
/// pattern matching on slices looked kinda weird; I had assumed it'd be faster than calling a
/// function.
///
/// Benchmark on my computer: around 30us/iter (713MB/s)
pub fn part2_no_regex_bidir_add_directly_byte_lines_nz(input: &str) -> i64 {
    let mut result: i64 = 0;
    for line in input.as_bytes().split(|c| *c == b'\n') {
        for ix in 0..line.len() {
            if let Some(digit) = get_digit_nz(&line[ix..]) {
                result += 10 * digit;
                break;
            };
        }
...
fn get_digit_nz(at: &[u8]) -> Option<i64> {
    if let Some(c @ b'1'..=b'9') = at.first() {
        return Some(-(b'0' as i16 - *c as i16) as i64);
    } else {
        match at {
            s if s.starts_with(b"one") => return Some(1),
            s if s.starts_with(b"two") => return Some(2),
            s if s.starts_with(b"three") => return Some(3),
            s if s.starts_with(b"four") => return Some(4),
            s if s.starts_with(b"five") => return Some(5),
            s if s.starts_with(b"six") => return Some(6),
            s if s.starts_with(b"seven") => return Some(7),
            s if s.starts_with(b"eight") => return Some(8),
            s if s.starts_with(b"nine") => return Some(9),
            _ => return None,
        }
    }
}
...
pub fn main(bench: bool) {
    ...
    let fns: Vec<(&'static str, fn(&str) -> i64)> = vec![
        ...
        (
            "part2 (no regex, bidir, add directly, byte lines, nz)",
            part2_no_regex_bidir_add_directly_byte_lines_nz,
        ),
    ];
    for (name, f) in &fns {
